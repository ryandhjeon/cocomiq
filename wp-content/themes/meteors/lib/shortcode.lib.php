...
function tg_social_icons_func($atts, $content) {
	...
	{
		$return_html.='<li class="facebook"><a target="_blank" title="Facebook" href="'.esc_url($pp_facebook_username).'"><i class="fa fa-facebook"></i></a></li>';
	}
...
function tg_social_share_func($atts, $content) {
	...
	$return_html.='<h5>'.__( 'Share On', THEMEDOMAIN ).'</h5><br/><br/>';
	$return_html.='<ul>';
...
function tg_grid_gallery_func($atts, $content) {
	...
	if(!empty($images_arr))
	{
		...
		foreach($images_arr as $key => $image)
		{
			...
			$pp_lightbox_enable_caption = get_option('pp_lightbox_enable_caption');
			...
			if(!empty($pp_lightbox_enable_caption)) 
			{
	...
	{
		$return_html.= __( 'Empty gallery item. Please make sure you have upload image to it or check the short code.', THEMEDOMAIN );
	}
...
function tg_grid_portfolio_func($atts, $content) {
	...
	{
		$return_html.= __( 'Empty portfolio item. Please make sure you have created portfolio item or check the short code.', THEMEDOMAIN );
	}
	...
	if(!empty($sets_arr) && empty($term))
	{
	    $return_html.= '<ul class="portfolio_filters portfolio-main filter shortcode"> 
	    	<li class="all-projects active">
	    		<a data-target="'.esc_attr($portfolio_wrapper_id).'" class="active" href="javascript:;" data-filter="*">'.__( 'All', THEMEDOMAIN ).'</a>
...
function tg_accordion_func($atts, $content) {
	...
	$return_html = '<div class="'.esc_attr($close_class).'"><h3><a href="#">';
...
function googlefont_func($atts, $content) {
	...
	if(!empty($font))
	{
		...
		$return_html = '<div class="googlefont" style="font-family:'.$font.';font-size:'.esc_attr($fontsize).'px">'.$content.'</div>';
	}
