		...
		<div class="gravatar">
      	...
      	<div class="right">
			...
			<?php
				...
				{
			?>
					<h7><?php echo $comment->comment_author; ?></h7>
			<?php
				}
			?>
	...
	if(!empty($posts))
	{
		...
		$count_post = count($posts);
			...
			foreach($posts as $post)
			{
				...
				$return_html.= '<a href="'.get_permalink($post->ID).'">'.$post->post_title.'</a><div class="post_attribute">'.get_the_time('d M', $post->ID).'</div>';
				$return_html.= '</li>';
...
function tg_apply_builder($page_id, $post_type = 'page') {
