...
function ppb_header_func($atts, $content) {
	...
	if(!empty($title))
	{
		$return_html.= '<h2 class="ppb_title" style="'.esc_attr($custom_css_fontcolor).'">'.urldecode($title).'</h2>';
	}
...
function ppb_image_parallax_func($atts, $content) {
	...
	if(!empty($custom_css))
	{
		$return_html.= 'style="background-image:url('.esc_url($image).');'.esc_attr(urldecode($custom_css)).'" ';
	}
...
function ppb_image_half_fullwidth_func($atts, $content) {
	...
	if($align=='left')
	{
		...
		$return_html.= '<div class="one_half_bg content_middle animate" '.$content_custom_css.'>';
	...
	{	
		$content_custom_css.= 'style="padding:'.esc_attr($padding).'px;"';
		$return_html.= '<div class="one_half_bg content_middle animate textright" '.$content_custom_css.'>';
...
function ppb_portfolio_classic_func($atts, $content) {
	...
	{
		$return_html.= '<div class="portfolio_filter_wrapper three_cols gallery portfolio-content section content clearfix">';
		...
		foreach($portfolios_arr as $key => $portfolio)
		{
			...
			$return_html.= '<div class="one_third gallery3 filterable static animated'.($key+1).'">';
			...
			if(!empty($image_url[0]))
			{
			    ...
			    case 'External Link':
					...
					$return_html.= '<a target="_blank" href="'.esc_url($portfolio_link_url).'"><img src="'.esc_url($small_image_url[0]).'" alt=""/></a>';
        	    ...
        	    default:
		        	...
		        	$return_html.= '<a href="'.get_permalink($portfolio_ID).'"><img src="'.esc_url($small_image_url[0]).'" alt=""/></a>';
        	    ...
        	    case 'Image':
					...
					$return_html.= '<a data-title="'.esc_attr($portfolio->post_title).'" href="'.esc_url($image_url[0]).'" class="fancy-gallery"><img src="'.esc_url($small_image_url[0]).'" alt=""/></a>';
			    ...
			    case 'Youtube Video':
					...
					$return_html.= '<a title="'.esc_attr($portfolio->post_title).'" href="#video_'.$portfolio_video_id.'" class="lightbox_youtube"><img src="'.esc_url($small_image_url[0]).'" alt=""/></a>';
			...
			case 'Vimeo Video':
					...
					$return_html.= '<a title="'.esc_attr($portfolio->post_title).'" href="#video_'.$portfolio_video_id.'" class="lightbox_vimeo"><img src="'.esc_url($small_image_url[0]).'" alt=""/></a>';
			...
			case 'Self-Hosted Video':
			    ...
			    $preview_image = wp_get_attachment_image_src($image_id, 'large', true);
					...
					$return_html.= '<a title="'.esc_attr($portfolio->post_title).'" href="#video_self_'.$key.'" class="lightbox_vimeo"><img src="'.esc_url($small_image_url[0]).'" alt=""/></a>';
			...
			$return_html.= '<br class="clear"/><div id="portfolio_desc_'.$portfolio_ID.'" class="portfolio_desc portfolio3 filterable">';
...
function ppb_portfolio_classic_masonry_func($atts, $content) {
	...
	$return_html = '<div '.$sec_id.' class="ppb_portfolio '.esc_attr($size).' nopadding ';
		...
		foreach($portfolios_arr as $key => $portfolio)
		{
			...
			if(!empty($image_url[0]))
			{
        	    ...
        	    default:
		        	...
		        	$return_html.= '<a href="'.esc_url(get_permalink($portfolio_ID)).'"><img src="'.esc_url($small_image_url[0]).'" alt=""/></a>';
...
function ppb_portfolio_grid_func($atts, $content) {
	...
	if(!empty($portfolios_arr) && is_array($portfolios_arr))
	{
		...
		foreach($portfolios_arr as $key => $portfolio)
		{
			...
			//Begin display HTML
			$return_html.= '<div class="element grid classic3_cols">';
			$return_html.= '<div class="one_third gallery3 filterable gallery_type static animated'.($key+1).'">';
			...
			if(!empty($image_url[0]))
			{
			    ...
			    case 'External Link':
		            ...
		            $return_html.= '<div class="portfolio_title">';
		            $return_html.= '<div class="table">';
        			$return_html.= '<div class="cell">';
		            $return_html.= '<h5>'.$portfolio->post_title.'</h5>';
					$return_html.= '<div class="post_detail">'.$portfolio->post_excerpt.'</div>';
			        $return_html.= '</div>';
			        $return_html.= '</div>';
        	    ...
        	    case 'Image':
					...
					$return_html.= '<a data-title="'.esc_attr($portfolio->post_title).'" href="'.$image_url[0].'" class="fancy-gallery">';
					$return_html.= '<img src="'.esc_url($small_image_url[0]).'" alt=""/>';
			...
			case 'Self-Hosted Video':
			    	...
			    	$return_html.= '<a title="'.esc_attr($portfolio->post_title).'" href="#video_self_'.$portfolio_video_id.'" class="lightbox_vimeo">';
					$return_html.= '<img src="'.esc_url($small_image_url[0]).'" alt=""/>';
	...
	if(!empty($posts_arr) && is_array($posts_arr))
	{
		...
		foreach($posts_arr as $key => $ppb_post)
		{
		    ...
		    switch($post_format)
			{
			    ...
			    <div class="post_header quote">
			    	<div class="post_quote_title grid">
			    		<div class="post_detail">
			    	    	'.get_the_time('d M', $ppb_post->ID).'&nbsp;';
			    		    
						$post_categories = wp_get_post_categories($ppb_post->ID);
						if(!empty($post_categories))
						{
						 	$return_html.= __( 'In', THEMEDOMAIN ).'&nbsp;';
				...
				$return_html.= $ppb_post->post_content.'
			    	</div>
			    </div>';
				...
				$return_html.= pp_substr(get_excerpt_by_id($ppb_post->ID), 65);
		        break;
		    }
...
function ppb_fullwidth_button_func($atts, $content) {
	...
	$return_html.= '<a href="'.esc_url($link_url).'" class="button fullwidth ppb" ';
	
	if(!empty($bgcolor))
	{
		$return_html.= 'style="background-color:'.esc_attr($bgcolor).'"';
	}
	...
	if(!empty($subtitle))
	{
		$return_html.= '<div class="subtitle" style="color:'.esc_attr($fontcolor).';">'.urldecode($subtitle).'</div>';
	}
	...
	$return_html.= '<i class="fa fa-long-arrow-right" style="color:'.esc_attr($fontcolor).';"></i>';
	$return_html.= '</a>';
	$return_html.= '</div>';
...
function ppb_team_column_func($atts, $content) {
	...
	if(!empty($team_arr) && is_array($team_arr))
	{
		...
		foreach($team_arr as $key => $member)
		{
			...
			{
				$return_html.= '<div class="post_img team"><img class="team_pic animated" data-animation="bigEntrance" src="'.esc_url($small_image_url[0]).'" alt=""/></div>';
			}
            ...
            {
            	$return_html.= '<div class="team_position" ';
...
function ppb_contact_map_func($atts, $content) {
	...
	$return_html.= '<div class="page_content_wrapper floatleft nopadding"><div class="one_half_bg contact_form" ';
    ...
    if(is_array($pp_contact_form) && !empty($pp_contact_form))
    {
        foreach($pp_contact_form as $form_input)
        {
        	switch($form_input)
        	{
    				...
    				<textarea id="message" name="message" rows="7" cols="10" class="required_field" style="width:96%;" placeholder="'.__( 'Message *', THEMEDOMAIN ).'"></textarea>
    				';	
    ...
    $return_html.= '<br/><br/><div class="contact_submit_wrapper">
    	<input id="contact_submit_btn'.$custom_id.'" name="contact_submit_btn'.$custom_id.'" type="submit" class="solidbg" value="'.__( 'Send', THEMEDOMAIN ).'" ';
    ...
    if(is_array($pp_contact_form) && !empty($pp_contact_form))
    {
        foreach($pp_contact_form as $form_input)
        {
        	switch($form_input)
        	{
    				...
    				$return_html.= '<label for="message">'.__( 'Message *', THEMEDOMAIN ).'</label>
    				<textarea id="message" name="message" rows="7" cols="10" class="required_field" style="width:95.5%;" placeholder="'.__( 'Message *', THEMEDOMAIN ).'"></textarea>
    				';	
